...
class GameState:
    def __init__(self, player, current_room, world_rooms, world_items,
                 item_flags, hammerspace_flags):
        self.player = player
...
# the room's locked state to view the unlocked state.
    ...
    def unlocked(self, item):
        if "key_item" in self.world_rooms[self.current_room]:
            ...
            if activated_key in self.item_flags and self.item_flags[
                    activated_key]:
                return True
            elif activated_key in self.hammerspace_flags and self.hammerspace_flags[
                    activated_key]:
                return True
            elif "inspect_key" in self.world_rooms[
                    self.current_room] and activated_key == item:
                return True
            else:
                return False
        else:
...
# Check if the item is valid, as a regular item in the room, or as a key item.
    ...
    def valid_item(self, item_name, room_items):
        ...
        if item_name in room_items:
            return True
        elif "key_item" in self.world_rooms[
                self.current_room] and self.world_rooms[
                    self.current_room]["key_item"] == item_name:
            return True
        else:
    ...
    def inspect(self, item):
        ...
        if "items" in self.world_rooms[
                self.current_room] or "specific_item" in self.world_rooms[
                    self.current_room]:
                ...
                if self.world_rooms[self.
                    ...
                    if item in self.item_flags:
                        if self.item_flags[item]:
                            ...
                            self.player.setMsg(
                                self.world_rooms[self.current_room]
                    ...
                    if item in self.world_rooms[self.
                                                current_room]["death_msg"]:
                        self.player.setMsg(self.world_rooms[self.current_room]
                                           ["death_msg"][item])
                        self.player.kill_player()
                    else:
                        self.player.setMsg(
                            self.world_rooms[self.current_room]["death_msg"]
                            ["11DEAFAULTDEATHMSG11"])
                        self.player.kill_player()
            ...
            elif self.valid_item(item, self.world_rooms[self.current_room]
                                 ["items"]) and self.unlocked(item):
                item = item.lower()
                print(item)
...
# has not been inspected before.
    ...
    def change_item_state(self, item):
        ...
        elif (item in self.hammerspace_flags) and (self.hammerspace_flags[item]
                                                   is False):
            self.player.addScore()
    ...
    def determine_effect(self, item):
        if "effect" in self.world_items[item]:
            ...
            # inspect_prereq: Kill player if player does not inspect prereq item; regular inspect otheriwse
                ...
                if (self.world_items[item]["prereq"] in self.item_flags) and (
                        self.item_flags[self.world_items[item]["prereq"]]):
                    self.player.setMsg(self.world_items[item]["item_desc"])
                    self.change_item_state(item)
                elif (self.world_items[item]["prereq"] in self.
                      hammerspace_flags) and self.hammerspace_flags[
                          self.world_items[item]["prereq"]]:
                    self.player.setMsg(self.world_items[item]["item_desc"])
                    self.change_item_state(item)
                else:
    ...
    def move(self, new_room):
        try:
            ...
            if self.world_rooms[self.current_room]["connections"] is not None:
                if new_room in self.world_rooms[self.
                                                current_room]["connections"]:
                    self.current_room = new_room
                    self.player.setMsg()
                else:
                    self.player.setMsg("'" + new_room + "'" +
                                       " is not accessible from here.")
            else:
                self.player.setMsg(
                    "The sound you make in moving around alerts HER to your presence. In her manic rampage, she shreds you to pieces. GAME OVER."
                )
                self.player.kill_player()
        except (AttributeError, TypeError):
            self.player.setMsg(
